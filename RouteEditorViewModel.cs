...
using RouteLoadoutApp.Services;
using Microsoft.Maui.Devices.Sensors; // Needed for Geocoding APIs

namespace RouteLoadoutApp.ViewModels;
...
public partial class RouteEditorViewModel : ObservableObject
{
	...
	private async Task AddWaypointAsync()
	{
		...
		{
			await Shell.Current.DisplayAlert("Error", "Please enter an address or location name.", "OK");
			return;
		}
		...
		else
		{
			...
			catch (Exception ex)
			{
				...
				await Shell.Current.DisplayAlert("Search Failed", "Could not find coordinates for that address. Check your connection.", "OK");
				return;
			}
		}
		...
		{
			await Shell.Current.DisplayAlert("Not Found", "Address not found. Try adding a city or zip code to the text.", "OK");
		}
	}
    ...
    private async Task SaveLoadoutAsync()
    {
        ...
        await Shell.Current.DisplayAlert("Success", "Loadout saved successfully!", "OK");
    }
